...
export default function Path({link, handlePathHover, handlePopupClose, activeTeams}: {
    ...
    const [{strokeWidth, strokeOpacity}, api] = useSpring(() => (strokeProperties));
    ...
    function onHover(event: React.MouseEvent<SVGPathElement, MouseEvent>) {
        api.start({strokeWidth: strokeProperties.strokeWidth * 1.75, strokeOpacity: 1});
    ...
    return <animated.path
        ...
        className={!searching || active ? teamColor(link.target.data.team_name) : "stroke-stone-500"}
